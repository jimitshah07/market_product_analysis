...
# ── Analytics Helpers ─────────────────────────────────────────────────────────
_CHAINS = {
    "starbucks", "costa", "ccd", "cafe coffee day", "tim horton", "tim hortons", "mcdonald's", "mcdonalds",
    "burger king", "subway", "kfc", "domino's", "dominos", "pizza hut", "barista", "third wave",
    "blue tokai", "chroma", "reliance digital", "vijay sales", "apple store", "samsung",
    "walmart", "target", "ikea", "h&m", "zara", "decathlon", "tata cli", "amazon", "flipkart"
}

def is_chain(name: str) -> bool:
    if not name: return False
    n = name.lower()
    return any(c in n for c in _CHAINS)
...
def scrollable_frame(parent, bg=None):
    ...
    def _on_mousewheel(event):
        ...
        # Simple scroll: if the outer frame is visible, scroll the canvas
    ...
    # Bind to canvas and all future children of inner
    canvas.bind("<MouseWheel>", _on_mousewheel, add="+")
    inner.bind("<MouseWheel>", _on_mousewheel, add="+")
    # For future widgets, we rely on the fact that most widgets don't have their own scroll logic
    # But a better way is to bind to the outer frame and use winfo_containing
    # Let's try the more robust containment check again but with parent walking
    
    def _robust_scroll(event):
        curr = outer.winfo_containing(event.x_root, event.y_root)
        while curr:
            if curr == outer:
                canvas.yview_scroll(-1 * (event.delta // 120), "units")
                break
    ...
    outer.winfo_toplevel().bind("<MouseWheel>", _robust_scroll, add="+")
...
### Extraction Logic
1. product: The specific item or restaurant name being searched.
2. location: Target city or region.
3. price_range: The targeted SELLING PRICE to consumer (NOT marketing budget).
4. is_restaurant: Boolean (true if the query is about food, dining, cuisine, or a specific restaurant name).
...
Output: {{"product": "headphones", "location": "Delhi", "price_range": [1800, 2200], "is_restaurant": false}}

User: "Find Italian restaurants in Mumbai"
Output: {{"product": "Italian", "location": "Mumbai", "price_range": null, "is_restaurant": true}}

User: "Bastian in Mumbai"
Output: {{"product": "Bastian", "location": "Mumbai", "price_range": null, "is_restaurant": true}}
...
class MapsDataFetcher:
    ...
    def fetch_places(product: str, location: str = "Ahmedabad",
        ...
        def _fetch(engine="google_maps"):
            q = (f"{product} in {location}" if is_restaurant
                 else f"{product} shop in {location}")
            params = {
                "engine": engine, "q": q,
                "api_key": SERPAPI_KEY, "hl": "en",
            }
            ...
            with urllib.request.urlopen(
                f"https://serpapi.com/search?{query_str}", timeout=25
            ) as r:
                return json.loads(r.read().decode())

        data = with_retry(lambda: _fetch("google_maps"))
        
        # Strategy 1: Check local_results
        results = data.get("local_results", [])
        
        # Strategy 2: Check place_results (if SerpApi redirects to a single place)
        if not results and data.get("place_results"):
            results = [data["place_results"]]
            
        # Strategy 3: Fallback to google_local engine if no results found
        if not results:
            log.info(f"google_maps returned 0 results for '{product}'. Trying google_local fallback...")
            data = with_retry(lambda: _fetch("google_local"))
            results = data.get("local_results", [])

        out  = []
        for idx, place in enumerate(results[:15]):
            ...
            # Map name/title correctly depending on engine
            name = place.get("title", place.get("name", "Unknown"))
            
            maps_link = place.get("link", "") or (
                "https://www.google.com/maps/search/" +
                urllib.parse.quote(name + " " + location)
            )
            ...
            out.append({
                ...
                "location":        place.get("address", place.get("address", location)),
    ...
    @staticmethod
    def fetch_photos(place_id: str, data_id: str = "", restaurant_name: str = "", location: str = "") -> List[Dict]:
        """Fetch photos with ultra-resilient fallbacks."""
        lookup_id = place_id or data_id or restaurant_name
        if not lookup_id:
            return []
        ck = cache_key("photos_v7", lookup_id + (location or ""))
        cached = get_cache(ck)
        if cached:
            return cached
        ...
        def _serpapi_fetch(param_name: str, id_val: str) -> List[Dict]:
            if not id_val or not SERPAPI_KEY:
                return []
            ...
            for p in data.get("photos", [])[:100]:
                ...
                url = url.strip()
                if not url.lower().startswith("http") or url in seen: continue
                seen.add(url)
                photos.append({
                    "url":      url,
        ...
        if SERPAPI_KEY:
            for param, id_val in [("data_id", data_id), ("data_id", place_id), ("place_id", place_id)]:
        ...
        # ── Strategy 3: Direct Scrape ─────────────────────────────────────────
        if not result and place_id:
            try:
                gmap_url = f"https://www.google.com/maps/place/?q=place_id:{place_id}"
                resp = requests.get(gmap_url, headers={"User-Agent": "Mozilla/5.0"}, timeout=20)
                seen = set()
                for m in re.finditer(r'https://lh[3-6]\.googleusercontent\.com/[^"\'\\s>]+', resp.text):
                    u = m.group(0).split("\\")[0].split('"')[0]
                    if "=s" in u: u = re.sub(r'=s\d+', '=s800', u)
        ...
        # ── Strategy 4: High-Power DuckDuckGo Intelligence ────────────────────
        # If we have very few photos, or the user specifically wants power, we do this.
        if len(result) < 15 and (restaurant_name or location):
            queries = [
                (f"{restaurant_name} {location} restaurant photos", "other"),
                (f"{restaurant_name} {location} restaurant menu card price list", "menu"),
                (f"{restaurant_name} {location} restaurant interior ambiance decor", "ambi")
            ]
            
            seen_urls = {p["url"] for p in result}
            
            for q_str, q_tag in queries:
                try:
                    q_enc = urllib.parse.quote(q_str)
                    # Get vqd
                    r1 = requests.get(f"https://duckduckgo.com/?q={q_enc}&iax=images&ia=images", timeout=10)
                    vqd_m = re.search(r'vqd=([\d-]+)', r1.text)
                    if not vqd_m:
                        vqd_m = re.search(r"vqd=['\"]([\d-]+)['\"]", r1.text)
                    if not vqd_m: continue
                    vqd = vqd_m.group(1)
                    
                    # Fetch images
                    api = f"https://duckduckgo.com/i.js?q={q_enc}&o=json&p=1&s=0&u=bing&f=,,,&l=us-en&vqd={vqd}"
                    r2 = requests.get(api, headers={"User-Agent": "Mozilla/5.0"}, timeout=12)
                    for item in r2.json().get("results", [])[:15]:
                        u = item.get("image", "")
                        if u and u.startswith("http") and u not in seen_urls:
                            seen_urls.add(u)
                            result.append({
                                "url": u, 
                                "tag": q_tag, # Pre-tag the result!
                                "title": item.get("title", ""), 
                                "category": q_tag
                            })
                except Exception as e:
                    log.warning(f"DDG fallback power-stage ({q_tag}) fail: {e}")
...
_AMBI_KW = {
    "inside", "outside", "storefront", "entrance", "facade", "building", "signage", "architecture",
    "lighting", "view", "terrace", "garden", "lobby", "reception", "dining area", "sofa",
    "wall", "ceiling", "floor", "parking", "street", "night", "day", "entrance", "hall",
    "rooftop", "counter", "booth", "bar", "staff", "setup", "vibe", "interior", "exterior",
    "decor", "seating", "table", "chair", "lamp", "window", "door", "stair", "plant", "frame",
    "art", "mirror", "bathroom", "toilet", "washroom", "kitchen", "counter", "chef", "decor",
    "ambience", "atmosphere", "outdoor"
}

_MENU_KW = {
    "menu", "rate", "price", "list", "card", "bill", "invoice", "order", "scan", "qr", "digital",
    "pricing", "cost", "item", "dish list", "food list", "drinks", "beverages", "wine list",
    "cocktail", "dessert menu", "lunch menu", "dinner menu", "special", "combo", "offer",
    "book", "catalogue", "options", "tariff", "ordering"
}

_FOOD_KW = {
    "food", "dish", "plate", "meal", "cuisine", "delicious", "tasty", "yummy", "cook", "serve",
    "eat", "hungry", "lunch", "dinner", "breakfast", "snack", "starter", "main course", "dessert",
    "pizza", "burger", "pasta", "biryani", "taco", "nacho", "burrito", "enchilada", "curry", "roti",
    "paneer", "chicken", "mutton", "fish", "prawn", "sushi", "noodle", "rice", "soup", "salad",
    "kebab", "tandoori", "masala", "samosa", "fry", "fries", "naan", "dosa", "idli", "vada", "thali"
}
...
def _classify_heuristic(url: str, tag: str = "", title: str = "") -> str:
    ...
    # Priority 1: Direct Menu Clues
    if any(kw in combined for kw in _MENU_KW): return "menu"
    
    # Priority 2: Direct Ambiance Clues
    if any(kw in combined for kw in ["storefront", "facade", "building", "signage", "entrance", "outside", "interior", "exterior", "ambiance", "ambience"]):
        return "ambi"
        
    # Priority 3: Google Maps specific URL patterns for user-uploaded types
    if "p/AF1Q" in url:
        # Most /p/AF1Q are user photos. If no interior keywords, assume food
        if any(kw in combined for kw in _AMBI_KW): return "ambi"
        return "food"
    
    # Priority 4: Food keywords
    if any(kw in combined for kw in _FOOD_KW): return "food"
    
    # Priority 5: Ambiance keywords
    if any(kw in combined for kw in _AMBI_KW): return "ambi"
...
def categorize_photos(photo_dicts: List[Dict],
    ...
    """Two-stage categorisation: 1. Fast heuristics, 2. AI vision (caller handles AI)."""
    buckets: Dict[str, Any] = {
        "food": [], "menu": [], "ambi": [], "other": [], "unclassified_data": []
    }
    ...
    for pd in photo_dicts:
        ...
        # Layer 0: Pre-tagged data (from High-Power DDG Intelligence)
        if cat in ["menu", "ambi", "food"]:
            buckets[cat].append(url); continue

        # Layer 1: Heuristics (High Confidence)
        h_res = _classify_heuristic(url, tag, title)
        if h_res != "other":
            buckets[h_res].append(url); continue

        # Layer 2: Native API Tags
        combined_tags = f"{tag} {cat}".strip()
        if any(t in combined_tags for t in ["menu","price","list","order"]):
            buckets["menu"].append(url); continue
        if any(t in combined_tags for t in ["interior","exterior","ambiance","ambience","seating","decor",
                                            "building","outdoor","indoor","rooftop","entrance","facade","storefront"]):
            buckets["ambi"].append(url); continue
        if any(t in combined_tags for t in ["food","dish","meal","drink","plate","eat","cuisine"]):
            buckets["food"].append(url); continue

        buckets["unclassified_data"].append(pd)
...
class RestaurantPanel:
    ...
    def _build(self):
        ...
        field(0, "🍴  Cuisine / Restaurant", self.cuisine_var)
    ...
    def show(self, cuisine=None, location=None):
        if cuisine:  self.cuisine_var.set(cuisine)
        if location: self.loc_var.set(location)
        self.win.deiconify(); self.win.lift(); self.win.focus_force()
        if cuisine: self._search()
    ...
    def _search(self):
        ...
        if not cuisine:
            messagebox.showwarning("Missing Input",
                "Please enter a cuisine or restaurant name.", parent=self.win); return
        
        # Smart query: don't force 'restaurant' if it's already there or looks like a name
        query = f"{cuisine} {vibe}".strip()
        
        for w in self.results_inner.winfo_children(): w.destroy()
        tk.Label(self.results_inner,
                 text=f"⏳  Searching '{cuisine}' in {location}…",
    ...
    def _on_success(self, data, cuisine, location):
        ...
        status_color = SUCCESS if data else WARNING
        self.status_lbl.config(
            text=f"{'✓' if data else '⚠'}  {len(data)} results for '{cuisine}' in {location}",
            fg=status_color)
...
class SortPreferenceDialog:
    OPTION_META = {
        "relevance": {
            "icon": "🎯", "label": "Relevance",
            "desc": "Google's default best-match\nShows big brands and chains",
            "color": "#3b82f6",
        },
        "nearest": {
            "icon": "📍", "label": "Nearest",
    ...
    def _build(self, current):
        ...
        # ── Cards ─────────────────────────────────────────────────────────
        cards_frame = tk.Frame(self.win, bg=T("BG"), pady=20, padx=25)
...
def sort_maps_data(data: List[Dict], preference: str) -> List[Dict]:
    if not data:
        return data
    if preference == "relevance":
        # Data is already fetched in native Google relevance order
    ...
    def _show_photo_gallery(self):
        ...
        def _download_image(url: str):
            ...
            for attempt in range(3):
                try:
                    resp = requests.get(
                        url, headers=_HEADERS, timeout=20,
                    ...
                    # More permissive check: Just ensure it's not HTML start
                    if data.strip().startswith(b"<!DOCTYPE") or data.strip().startswith(b"<html"):
                        return None
                        
                    return data
                except Exception as exc:
                    log.debug(f"Image download attempt {attempt+1} failed: {exc}")
                    if attempt < 2:
        ...
        def _render_section(buckets, all_urls, refining=False):
            ...
            # --- Ambiance Catch-All ---
            # If ambiance is empty, look for interior/seating/decor in 'other'
            # OR just move ALL 'other' to Ambiance so the user sees them
            if other_u:
                ambi_u.extend(other_u)
                other_u = []
            ...
            if refining:
                status_msg += "  ·  ✨ AI Refining (Llava)..."
            else:
                status_msg += "  ·  ✓ Categorisation Complete"
            ...
            if OLLAMA_AVAILABLE and menu_u:
                def _do_ocr():
                    # If already extracted, show immediately
                    cached = self.shop.get("ai_menu_result")
                    if cached:
                        _show_res(cached)
                        return
                    ...
                    def _thread():
                        ...
                        self.shop["ai_menu_result"] = res
                        if gw.winfo_exists():
                            gw.after(0, lambda: _show_res(res))
                    threading.Thread(target=_thread, daemon=True).start()

                def _show_res(res):
                    ocr_btn.config(text="✨ View AI-Analyzed Menu", state="normal")
                    pw = tk.Toplevel(gw)
                    pw.title(f"✨ AI Menu — {shop_name}")
                    pw.geometry("650x750")
                    pw.configure(bg=T("BG"))
                    f = tk.Frame(pw, bg="#7c3aed", pady=16)
                    f.pack(fill="x")
                    tk.Label(f, text=f"✨ AI Menu Analysis — {shop_name}", font=("Segoe UI Variable", 13, "bold"), bg="#7c3aed", fg="#fff").pack()
                    txt = tk.Text(pw, font=FONT_BODY, bg=T("SURFACE"), fg=T("TEXT1"), wrap="word", padx=20, pady=20, relief="flat")
                    txt.pack(fill="both", expand=True, padx=16, pady=16)
                    txt.insert("1.0", res)
                    txt.config(state="disabled")

                ocr_btn = tk.Button(imn, text="✨ View AI-Analyzed Menu" if self.shop.get("ai_menu_result") else "✨ AI: Analyse Menu Photos",
                                    font=("Segoe UI Variable", 9, "bold"), bg="#7c3aed", fg="#fff",
                                    relief="flat", cursor="hand2", padx=16, pady=8,
                                    command=_do_ocr)
                ocr_btn.pack(pady=12)

            # Menu Tab
            if not menu_u:
                mlink = self.shop.get("online_menu_link")
                if mlink:
                    for c in imn.winfo_children(): c.destroy()
                    tk.Label(imn, text="No menu photos found, but we found a digital link:", 
                             font=FONT_BODY, bg=T("BG"), fg=T("TEXT2")).pack(pady=(40, 15))
                    _neon_btn(imn, "  🔗  Open Official Menu (Zomato/Web)  ", SUCCESS, "#ffffff",
                              lambda: webbrowser.open(mlink), font=("Segoe UI Variable", 10, "bold")).pack()
                    tk.Label(imn, text=f"\nSource: {mlink[:70]}...", 
                             font=FONT_SMALL, bg=T("BG"), fg=T("TEXT3")).pack()
                elif refining:
                    _render_grid(imn, [], "✨ AI is currently scanning your photos for a menu...\nThis usually takes 15-30 seconds on CPU.")
                else:
                    _render_grid(imn, [], "No menu photos found for this restaurant.")
            else:
                _render_grid(imn, menu_u, "No menu photos available.")
        ...
        # ── Background worker — fetch + categorise, never blocks UI ───────────
        def worker():
            # 1. Start with the thumbnail (guaranteed first photo)
            photo_dicts = []
            thumb = self.shop.get("thumbnail", "")
            if thumb and isinstance(thumb, str) and thumb.startswith("http"):
                photo_dicts.append({"url": thumb, "tag": "food", "title": "thumbnail", "category": "food"})

            # 2. Fetch deep photo list (GMap -> Scrape -> DDG Intelligence)
            try:
                loc_val = self.parent.loc_var.get() if hasattr(self.parent, 'loc_var') else ""
                fetched = MapsDataFetcher.fetch_photos(place_id, data_id, shop_name, loc_val)
                for p in fetched:
                    if not any(x["url"] == p["url"] for x in photo_dicts):
                        photo_dicts.append(p)
            except Exception as exc:
                log.warning(f"fetch_photos error: {exc}")

            if not photo_dicts:
                if gw.winfo_exists():
                    gw.after(0, lambda: status_var.set("⚠  No photos found for this restaurant."))
                return

            # ── PRIORITY: Menu Link Discovery ──────────────────────────────────
            # Try to find a Zomato/Swiggy/Website menu link if not already found
            if not self.shop.get("online_menu_link"):
                try:
                    lq = urllib.parse.quote(f"{shop_name} {loc_val} restaurant menu zomato swiggy")
                    lresp = requests.get(f"https://duckduckgo.com/html/?q={lq}", 
                                         headers={"User-Agent": "Mozilla/5.0"}, timeout=10)
                    links = re.findall(r'href="(https?://[^"]+)"', lresp.text)
                    for l in links:
                        l_low = l.lower()
                        if any(k in l_low for k in ["zomato.com", "swiggy.com", "magicpin.in", "easydiner.com"]) and "menu" in l_low:
                            self.shop["online_menu_link"] = l
                            break
                        if l_low.endswith(".pdf") and "menu" in l_low:
                            self.shop["online_menu_link"] = l
                            break
                except Exception: pass

            # ── Categorisation — Stage 1 (Fast Heuristics + Pre-tagged DDG) ──
            all_urls = [pd["url"] for pd in photo_dicts]
            buckets  = categorize_photos(photo_dicts, shop_name)
            unclassified = buckets.pop("unclassified_data", [])
            
            # Show results IMMEDIATELY
            if gw.winfo_exists():
                gw.after(0, lambda: _render_grid(ial, all_urls, "No photos available."))
                gw.after(0, lambda: _render_section(buckets, all_urls, refining=OLLAMA_AVAILABLE))

            # ── Stage 2: Background Intelligence (OCR + Vision) ──
            if OLLAMA_AVAILABLE:
                # 2.1 Auto-Menu Extraction
                if buckets.get("menu"):
                    def _auto_ocr():
                        res = _ollama_read_menu_photos(buckets["menu"], shop_name)
                        self.shop["ai_menu_result"] = res
                        log.info(f"Auto-Menu OCR complete for {shop_name}")
                    threading.Thread(target=_auto_ocr, daemon=True).start()

                # 2.2 Slow AI Vision for unclassified items
                if unclassified:
                    for pd in unclassified[:10]:
                        if not gw.winfo_exists(): break
                        ai_cat = _ollama_classify_image(pd["url"], shop_name)
                        if ai_cat in buckets:
                            buckets[ai_cat].append(pd["url"])
                            if gw.winfo_exists():
                                gw.after(0, lambda: _render_section(buckets, all_urls, refining=True))
                
                # Move remaining to other
                rem_urls = [x["url"] for x in unclassified[10:]]
                buckets["other"].extend(rem_urls)
                if gw.winfo_exists():
                    gw.after(0, lambda: _render_section(buckets, all_urls, refining=False))
            else:
                if unclassified:
                    rem_urls = [x["url"] for x in unclassified]
                    buckets["other"].extend(rem_urls)
                    if gw.winfo_exists():
                        gw.after(0, lambda: _render_section(buckets, all_urls, refining=False))
...
class MarketResearchApp(tk.Tk):
        ...
        def on_chosen(pref: str):
            ...
            if self.maps_data:
                ...
                self._add_ai_message(
                    f"Shops re-sorted: {label_map.get(pref, 'Relevance')}  ✓", fg=ACCENT
                )
    ...
    def _build_main_panel(self):
        ...
        self.loc_var   = tk.StringVar(value="Ahmedabad")
        self.search_mode = tk.StringVar(value="Auto-Detect")
        
        tk.Label(right, text="Mode:", font=("Segoe UI", 8, "bold"),
                 bg=T("SURFACE2"), fg=T("TEXT2")).pack(side="left", padx=(0, 4))
        self.mode_combo = ttk.Combobox(right, textvariable=self.search_mode, 
                                       values=["Auto-Detect", "Market Research", "Restaurant Analysis"], 
                                       width=20, state="readonly")
        self.mode_combo.pack(side="left", padx=(0, 10), ipady=3)

        field("Search For:", self.query_var, 18)
        field("Location:",    self.loc_var,   13)
    ...
    def _on_llm_success(self, result: dict):
        ...
        is_rest  = result.get("is_restaurant", False)
        ...
        if mode == "Restaurant Analysis":
            ...
            is_rest = False
        # otherwise use is_rest from LLM result (Auto-Detect)

        if is_rest:
            self._add_ai_message(
                f"🍽️  Restaurant Search detected: \"{product}\" in {self.last_location}\n"
                "Opening Restaurant Competitor Analysis...", fg=ACCENT
            )
            self.send_btn.config(state="normal", bg=PRIMARY)
            self._open_restaurant_panel()
            self._rest_panel.show(cuisine=product, location=self.last_location)
            return
    ...
    def _run_all(self):
        ...
        # Restaurant / Cuisine detection
        rest_kws = {
            "restaurant", "cafe", "bistro", "bakery", "dhaba", "food", "cuisine", "dining", "eatery",
            "pizza", "burger", "pasta", "sushi", "biryani", "thali", "sweets", "coffee", "tea", "chai",
            "bar", "pub", "lounge", "club", "grill", "steakhouse", "kebab", "tandoori", "momos",
            "punjabi", "chinese", "italian", "mexican", "thai", "gujarati", "south indian", "north indian",
            "continental", "dessert", "ice cream", "waffle", "sandwich", "roll", "shawarma", "bastian"
        }
        mode = self.search_mode.get()
        if mode == "Restaurant Analysis":
            is_restaurant = True
            ...
            # Auto-Detect mode
            is_restaurant = any(kw in query.lower() for kw in rest_kws)
        
        # If it's a restaurant search, switch to the restaurant panel
        if is_restaurant:
            self._open_restaurant_panel()
            self._rest_panel.show(cuisine=query, location=location)
            return
    ...
    def _build_analytics(self, maps_valid=None, amzn_prices=None, play_rated=None,
        ...
        # ── Market Structure (Chain vs Local) ──────────────────────────────────
        if maps_valid:
            section_hdr("🏢", "Market Structure  —  Chain vs Local Brands",
                        "Dominance of big brands vs. boutique local shops", accent="#f59e0b")
            
            chains = []
            locals_ = []
            for c in self.maps_data:
                if is_chain(c["name"]) or c["review_count"] > 10000:
                    chains.append(c)
                else:
                    locals_.append(c)
            
            fig_ms = Figure(figsize=(12, 4.5), dpi=96)
            fig_ms.patch.set_facecolor(T("BG"))
            ax1 = fig_ms.add_subplot(1, 2, 1)
            ax2 = fig_ms.add_subplot(1, 2, 2)
            ax1.set_facecolor(T("SURFACE"))
            ax2.set_facecolor(T("SURFACE"))

            cv, lv = len(chains), len(locals_)
            if cv + lv > 0:
                ax1.pie([cv, lv], labels=["Chains", "Local"],
                        colors=["#3b82f6", "#10b981"], autopct='%1.1f%%',
                        startangle=140, textprops={'color': T("TEXT1"), 'fontsize': 8})
                ax1.set_title("Market Volume (Shop Count)", fontsize=10, fontweight="bold", color=T("TEXT1"))
            
            crev = sum(c["review_count"] for c in chains)
            lrev = sum(c["review_count"] for c in locals_)
            trev = (crev + lrev) or 1
            ax2.bar(["Chains Share", "Local Share"], [crev/trev*100, lrev/trev*100], 
                    color=["#3b82f6", "#10b981"], width=0.6)
            ax2.set_ylim(0, 100)
            ax2.set_ylabel("% of Total Reviews", fontsize=8, color=T("TEXT2"))
            ax2.set_title("Market Dominance (by Review Share)", fontsize=10, fontweight="bold", color=T("TEXT1"))
            ax2.grid(axis='y', linestyle='--', alpha=0.15, color=T("BORDER"))
